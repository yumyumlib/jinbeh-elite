...
FC pitch — all still unresolved. Additionally from AGENTS.md active projects: Chef
...
- **Deploy procedure used (zero-downtime-on-build):**
  1. `rsync` local → `/opt/jinbeh-elite/` (exclude node_modules,.next,.git,.Codex,
...
## 2026-05-15 — Archive backup system deployed + rsync .Codex exclusion
...
### rsync deploy command — MUST exclude .Codex

The `.Codex` directory (Cowork agent worktrees/data) can grow to 8-9GB.
